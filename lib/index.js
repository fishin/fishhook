    ...
    var schedule = new Cron.CronJob({
        ...
        onTick: function () {
            ...
            if (job.scm && job.scm.prs) {
                internals.checkJobPRs(jobId, null, function (prs) {
...
internals.checkJobPRs = function (jobId, token, cb) {

    var newPRs = [];
    internals.FishHook.settings.getPullRequests(jobId, token, function (prs) {
